...
```
FishController  →  只改鱼的局部 X（左右游动）
BGController    →  改 BG 根节点的世界 Y
BG/fishs 是 BG 的子物体  →  鱼自动与背景同速
```

鱼和背景本来就是同一棵树上的父子关系，位置只有一份，**永远不可能不同步**。
上浮时世界原路退回，同一批没被抓住的鱼会从上往下再经过一次，正好形成"下去躲鱼、上来抓鱼"的节奏。
...
MVC + 观察者 + 状态机 + 对象池 + 数据驱动。**每个 Controller 挂在自己的物体上，GameMgr 里只有引用。**

```
场景层级                          挂载的脚本
─────────────────────────────────────────────────────────
BG                    ← BGController       世界滚动 + 背景贴图循环
├── bgs               ← （3 张背景贴图）
└── fishs             ← FishSpawner        对象池 / 生成 / 回收 / 接管美术摆的鱼
                        FishController     左右游动 / 边界掉头 / 越界标记
hook                  ← HookController     横向跟随 + DOTween 纵向分段 + 碰撞仲裁
                        HookView           鱼线拉伸 / 钩子包围盒 / 挂鱼
GameMgr               ← GameMgr            状态机 + 驱动控制器 + 数据广播
HUDCanvas             ← HUDView            血条/分数/深度/渔获/提示（只订阅事件）
ResultCanvas          ← ResultPanel        结算面板
```

```
Assets/Scripts
├── Config/       GameConfig / FishConfig / DefaultGameData    数据层
├── Core/         GameMgr / GameState / GameEvent / EventMgr   总控层
├── Model/        GameModel                                    纯数据，不依赖 Unity
├── Entities/     Fish / Hook                                  运行时实体
├── Controller/   BG / Hook / Fish / FishSpawner / Input       逻辑层
...
private void Update()
{
    ...
    BroadcastIfChanged();                               // ③ 数据广播
}
```

没有 `Find`、没有 `AddComponent`、没有 `Init` 转发——控制器各自在 `Awake` 里自初始化
（配置走 `GameConfig.Get()` / `FishConfig.Get()` 缓存，相机走 `Camera.main`，兄弟组件走 `GetComponent`），
GameMgr 只在 Inspector 里拖 3 个引用。碰撞判定这类"鱼钩碰到鱼"的逻辑归 `HookController`。
...
只靠 `Awake` 的先后顺序不可靠（Unity 不保证组件间顺序），因此 `HookView.Prepare()` 做成幂等的显式初始化，
由 `HookController.Awake` 在移动鱼钩前主动调用。
...
`BG` 根节点带缩放 `(1.17, 1.26, 1)`，`fishs` 又是它的子物体，所以"屏幕上下边缘"不能直接当局部坐标用。
`ViewportUtil` 统一处理三个坐标系的换算：

```csharp
float worldY = cam.transform.position.y + ViewportUtil.HalfHeight(cam); // 世界
float localY = ViewportUtil.WorldYToLocalY(fishRoot, worldY);           // 父节点局部
ViewportUtil.MoveWorldX(transform, deltaWorldX);                        // 自动除以父缩放
```

**6. 被抓的鱼怎么从鱼群里摘出去**

`HookController` 只给鱼打 `IsCaught` 标记，`FishSpawner` 每帧自己 `PruneCaught()` 摘掉——
