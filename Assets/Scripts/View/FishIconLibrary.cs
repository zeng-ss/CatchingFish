...
namespace View
{
    ...
    ///
    /// 两级来源：
    ///   ① 优先用菜单「工具/捕鱼/6. 烘焙鱼的结算图标」生成的真实渲染图
    ///      （Resources/Icon/Fish/{预制体名}.png，透明底，相机正交侧拍）；
    ///   ② 没烘焙过时**绝不返回空**，而是现场按鱼种主题色画一张剪影兜底 ——
    ///      这样即使忘了跑菜单（或者要拿去展示的机器上没有图），结算界面也不会出现空白方块。
    ///
    /// 所有结果都缓存，一局最多生成一次；生成成本是 128×128 的像素循环，可以忽略。
    /// </summary>
